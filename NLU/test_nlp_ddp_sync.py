...
def train(args, dograd, model, optimizer, train_loader, test_loader,epoch, history, scheduler):
...
def worker(comm, args, model, encoded_dataset):
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, dograd, model, optimizer, train_loader, test_loader,epoch, history, scheduler)
        torch.save(history, './log_nlp/' + 'DDP_Hook_SYNC_world_' + str(args.world_size) + "_" + args.dataset_name +  '_worker_' + str( args.rank + 1) + '_' + args.model_name + '_history.pt')
